...
class Band:
    def __init__(self, start_freq, stop_freq, resolution):
        ...
        self.attributes['Start Frequency'] = start_freq
        self.attributes['Stop Frequency'] = stop_freq
        self.attributes['Resolution Bandwidth'] = resolution
        self.scans = []
    ...
    @property
    def resolution(self):
        return self.attributes['Resolution Bandwidth']

    @property
    def start_freq(self):
        return self.attributes['Start Frequency']

    @property
    def stop_freq(self):
        return self.attributes['Stop Frequency']

    @property
    def scan_data(self):
        data = []
        for scan in self.scans:
            data.append(scan.measurements)
        return np.array(data)

    @property
    def resolution_bandwidth(self):
        bandwidth = self.stop_freq - self.start_freq
        return float(bandwidth) / self.resolution
