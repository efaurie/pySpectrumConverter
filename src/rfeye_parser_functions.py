...
def read_structure(input_file, type_sequence):
    ...
    for c_type, variable_name in type_sequence:
        # Replaces '%(Variable Name)d' in the ctype with the value of 'Variable Name' (read previously)
        c_type = c_type % data

        try:
            chunk_size = struct.calcsize(c_type)
        except struct.error:
            print 'read_next {0} {1} {2} {3}'.format(chunk_size, c_type, variable_name, data)
        ...
        value = struct.unpack(c_type, chunk)

        # Need to figure out when the version switch happened
        # Then read version from RFEye header and replace value here
        version = 2.0
        if variable_name == 'data':
            value = decode_measurement_data(data['level'], value, version)
...
def decode_measurement_data(offset, encoded_values, version):
    if version <= 1.0:
        return decode_measurement_data_v1(offset, encoded_values)
    else:
        return decode_measurement_data_v2(offset, encoded_values)


def decode_measurement_data_v1(offset, encoded_values):
    return [((encoded_point/2.0) + offset - 127.5) for encoded_point in encoded_values]


def decode_measurement_data_v2(offset, encoded_values):
    return [((-1*(encoded_point / 2.0)) + offset) for encoded_point in encoded_values]
